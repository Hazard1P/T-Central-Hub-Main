...
export async function joinDurableRoom({ roomName = ROOM_NAME, identity = {}, steamUser = null } = {}) {
  ...
  const timestamp = nowIso();
  const id = String(identity.id || steamUser?.steamid || `guest-${Date.now().toString(36)}`);
  ...
  const playerRow = {
    ...
    identity_kind: identity.kind || (steamUser?.steamid ? 'steam' : 'guest'),
...
export async function updateDurablePlayer({ roomName = ROOM_NAME, id, token, snapshot = {} } = {}) {
  const admin = getSupabaseAdmin();
  if (!admin) return { ok: false, status: 503, error: 'DURABLE_MULTIPLAYER_UNAVAILABLE' };
...
export async function applyDurableAction({ roomName = ROOM_NAME, id, token, action = {} } = {}) {
  const admin = getSupabaseAdmin();
  if (!admin) return { ok: false, status: 503, error: 'DURABLE_MULTIPLAYER_UNAVAILABLE' };
