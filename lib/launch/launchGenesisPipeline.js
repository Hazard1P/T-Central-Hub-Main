...
export function runLaunchGenesisPipeline({ authContext, lobbyMode = 'private', playableAt = Date.now() } = {}) {
  const epoch = createEpochAnchor({ playableAt });
  const accountId = authContext?.accountId || authContext?.identity?.id || 'guest-server';
  ...
  const starAnchorBuild = createBuildAnchor(authContext, lobbyMode, {
    accountId,
    epochUnix: epoch.unix,
  });
