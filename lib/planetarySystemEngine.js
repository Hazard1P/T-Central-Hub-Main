import { classifyPlanetZone, createStellarProfile, periodYearsForOrbit, scaleOrbitForStar } from '@/lib/stellarPhysics';
...
export function buildEpochPlanetarySystem({
  ...
  const stellarProfile = createStellarProfile({
    seed: normalizedSeed,
    classHint: scope === 'hub' ? 'F8V' : 'G2V',
    label: starLabel,
  });
  const starNode = {
    key: starKey,
    ...
    position: center,
    color: stellarProfile.color || starColor,
    kind: 'solar',
  ...
  const orbiters = PLANET_BASELINES.map((planet, index) => {
    ...
    const scaledOrbitAu = scaleOrbitForStar(planet.radius * radiusScale, stellarProfile);
    const scaledRadius = 1.22 + Math.log(scaledOrbitAu + 1) * 3.05;
    const orbitalPeriodYears = periodYearsForOrbit(scaledOrbitAu, stellarProfile) * periodScale;
    const angularVelocity = clamp(0.9 / Math.pow(orbitalPeriodYears + 0.2, 0.36), 0.012, 0.82);
    const orbitTilt = ((planet.tilt * Math.PI) / 180) + tiltJitter;
    const eccentricity = clamp(planet.eccentricity + eccentricityJitter, 0, 0.32);
    const zone = classifyPlanetZone(scaledOrbitAu, stellarProfile);
    ...
    return {
      ...
      period: round(orbitalPeriodYears, 3),
      orbitalPeriodYears: round(orbitalPeriodYears, 3),
