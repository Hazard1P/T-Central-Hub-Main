...
export function buildEpochPlanetarySystem({
  ...
  serverAnchor = { host: 'localhost:3000', token: 'srv-local' },
} = {}) {
  ...
  const starNode = {
    ...
    description: scope === 'hub'
      ...
      : `Private Central.Star anchored to a rolling Unix epoch segment. It drives the player-local planetary ratios and keeps the sealed universe synchronized to the current epoch window while staying pinned to hosting anchor ${serverAnchor.host}.`,
    position: center,
    color: starColor,
    kind: 'solar',
