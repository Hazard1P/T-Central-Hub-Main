...
function inferBlackholeClass(node = {}) {
  if (node.associatedServer) return `server-linked:${node.associatedServer}`;
  if (node.target_server_id) return `server-linked:${node.target_server_id}`;
...
export function buildBlackholeBindingRegistry(nodes = []) {
  ...
  const activeByInstanceAndClass = new Map();
  ...
  nodes
    ...
    .forEach((node) => {
      ...
      // Enforce one route per blackhole key. First bind wins only within the same class key.
      if (routeRecord.target_server_id) {
        ...
        const instanceClassKey = `${blackholeClass}::${routeRecord.target_server_id}`;
        if (!activeByInstanceAndClass.has(instanceClassKey)) {
          activeByInstanceAndClass.set(instanceClassKey, routeRecord.blackhole_id);
        } else if (activeByInstanceAndClass.get(instanceClassKey) !== routeRecord.blackhole_id) {
