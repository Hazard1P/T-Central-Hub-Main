...
import { WORLD_SUB_SECTOR_ASSETS } from '@/lib/worldLayout';

export function buildGravitySources(graph) {
  const nodes = Array.isArray(graph?.nodes) ? graph.nodes : [];
  return createGravitySources(nodes.filter((node) => Array.isArray(node.position)), { subSectorAssets: WORLD_SUB_SECTOR_ASSETS });
}
