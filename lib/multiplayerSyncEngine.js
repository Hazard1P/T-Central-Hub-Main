...
export function resolveMultiplayerIdentity(steamUser = null, googleUser = null) {
  if (steamUser?.steamid) {
    return {
      id: `steam:${String(steamUser.steamid)}`,
  ...
  if (googleUser?.sub) {
    return {
      ...
      displayName: googleUser.name || googleUser.email || 'Google Pilot',
      authenticated: true,
      kind: 'google',
    };
  }
...
export function createPresenceSnapshot({ steamUser = null, googleUser = null, telemetry = null, scope = null, lobbyMode = 'hub' } = {}) {
