...
function buildContinuityReport(events = []) {
  ...
  for (const [playerId, playerEvents] of perPlayer.entries()) {
    const ordered = [...playerEvents].sort(sortEventsByTickOrTimestamp);
...
function sortEventsByTickOrTimestamp(a, b) {
...
export function validateSimulationSymmetry(
  ...
  const ordered = [...events].sort(sortEventsByTickOrTimestamp);
  const perPlayer = new Map();
  ...
  for (const [playerId, playerEvents] of perPlayer.entries()) {
    const sortedPlayerEvents = [...playerEvents].sort(sortEventsByTickOrTimestamp);

    for (let index = 1; index < sortedPlayerEvents.length; index += 1) {
      const previous = sortedPlayerEvents[index - 1];
      const current = sortedPlayerEvents[index];
