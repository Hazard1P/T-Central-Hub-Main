...
export async function getLatestSupportSubscriptionLedgerRecordBySteamId(steamid) {
  const admin = getSupabaseAdmin();
  if (!admin || !steamid) return null;
  ...
  const { data, error } = await admin
    ...
    .eq('provider', 'paypal')
    .eq('steamid', String(steamid))
    .eq('identifier_type', 'subscription')
    .order('updated_at', { ascending: false })
    .limit(1)
    .maybeSingle();

  if (error) throw new Error(`Unable to read support subscription ledger: ${error.message}`);
  return data ? toSupportLedgerRecord(data) : null;
}
