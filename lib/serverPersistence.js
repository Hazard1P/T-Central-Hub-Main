...
async function appendLocalAccountLedgerEntry(event) {
  await appendJsonRecord('player-account-ledger.json', event);
...
export async function ensurePlayerAccountLedger({ steamUser } = {}) {
  ...
  try {
    ...
    if (supabase) {
      ...
      if (created) {
        const { error: ledgerError } = await supabase.from('player_account_ledger').insert({
    ...
    let ledgerAppended = false;
    if (created) {
      const ledgerWrite = await appendLocalAccountLedgerEntry({
        account_key: profile.account_key,
        event_type: 'ACCOUNT_CREATED',
        event_payload: {
          provider: 'steam',
...
export async function persistPlayerProgression(snapshot) {
  ...
  const ledgerEvent = {
    ...
    eventType: PROGRESSION_SNAPSHOT_EVENT,
