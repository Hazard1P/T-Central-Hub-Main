...
export function createPrivateWorldAsset({ steamUser = null, lobbyMode = 'private', now = Date.now(), identity = null } = {}) {
  ...
  return {
    ...
    label: publicLabel,
    description: 'Singleplayer map asset with a private blackhole nested inside the local universe fabric. The anchor remains private and is keyed to a Unix-epoch network window whether the player is Steam-linked or using the guest fallback state.',
    position,
    blackholePosition,
    nodes: [
      {
