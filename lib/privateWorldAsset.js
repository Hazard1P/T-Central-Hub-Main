...
export function createPrivateWorldAsset({ steamUser = null, lobbyMode = 'private', now = Date.now(), identity = null } = {}) {
  ...
  const common = {
    ...
    encryptedCheckpointScope: `checkpoint:${privateScope}`,
  };

  const playableBridge = {
    playable: true,
    authority: 'server-verified-private-map-anchor',
    checkpointScope: `checkpoint:${privateScope}`,
    encryptedHandoff: true,
    allowedClients: ['desktop', 'steam', 'vr'],
  };
  ...
  const associatedServerBlackholes = PRIMARY_SERVER_ROUTES.slice(0, 4).map((server, index) => {
    ...
    return {
      ...
      playableBridge: {
        ...playableBridge,
        targetServerSlug: server.slug,
        humanAvatars: server.slug === 'arma3-cth',
        humanEntities: server.slug === 'arma3-cth',
        vrObserver: server.slug === 'arma3-cth',
        allowedServerActions: server.slug === 'arma3-cth'
          ? ['spawn_human_avatar', 'replicate_human_entity', 'attach_vr_observer', 'steam_quick_connect']
          : ['steam_quick_connect', 'continuity_restore'],
      },
      tags: ['private mesh', 'server-associated', 'benefactor anchor', server.family || server.game.toLowerCase(), server.slug === 'arma3-cth' ? 'human avatar vr bridge' : 'server route bridge'],
    };
  });
  ...
  return {
    ...
    label: publicLabel,
    description: 'Singleplayer map asset with independent epoch-rolling solar replicas, one central-star core, four server-associated private blackholes, and a playable Arma human-avatar / VR bridge nested inside the local universe fabric.',
    playableBridge,
    position,
    blackholePosition,
    nodes: [
      {
        ...
        playableBridge: {
          ...playableBridge,
          targetServerSlug: 'arma3-cth',
          humanAvatars: true,
          humanEntities: true,
          vrObserver: true,
        },
        tags: ['singleplayer', 'private map asset', 'unix epoch anchor', identityStatus, 'epoch replica lattice', 'arma playable anchor'],
      },
      {
        ...common,
        key: blackholeKey,
        ...
        tags: ['private mesh', 'epoch anchor', 'sealed blackhole', 'central mesh core', 'playable checkpoint relay'],
      },
    ...
    routeLinks: [
      ...
      ...associatedServerBlackholes.map((node, index) => ({
        ...
        weight: 0.72,
        internalOnly: true,
        playerTraversable: node.serverSlug === 'arma3-cth',
        privateOnly: true,
        playableBridge: node.playableBridge,
      })),
      ...planetarySystems
        .slice(1)
        .map((system, index) => ({
