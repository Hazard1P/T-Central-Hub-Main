...
const PRIMARY_WORLD_NODES = [
  ...
  { key: 'csis', label: 'Canada Strings of Intelligence Dispersal Dyson Sphere', address: 'Government of Canada / Canada Strings of Intelligence Dispersal defense core', description: 'Canada Strings of Intelligence Dispersal Dyson sphere synchronized to the Government of Canada authority route. Ring 1 handles intelligence dispersal outputs, Ring 2 handles ingress/egress server and external system database routing, and Ring 3 handles entropy regeneration for star singularity state. The sphere is system-owned, sealed to player construction, and reserved for game defense only.', position: [-6.8, 10.6, 5.8], color: '#8ff3ff', kind: 'dyson', priority: 7, tags: ['intelligence dispersal outputs', 'ingress egress routing', 'external database routing', 'entropy regeneration', 'foundation core', 'sealed defense'], route: 'https://www.canada.ca/en/security-intelligence-service.html', external: true, dysonProfile: 'csis', foundationBuilt: true, networkRole: 'foundation-security', systemOwned: true, playerInteractive: false, playerBuildable: false, requestSurface: false, routeSurface: 'sealed' },
  ...
  { key: 'affiliates', label: 'Affiliates', address: 'Dyson sphere', description: 'Affiliates Dyson sphere node.', position: [-10.6, -9.4, 6.4], color: '#ff9fd9', kind: 'dyson', priority: 5, foundationBuilt: true },
  {
    key: 'solar_system',
    label: 'Solar System',
    address: 'Sun + 9 planets',
    description: 'Solar system locked into the T-Central Hub zone with nine orbiting planets and declared as the playable reference map for web and server handoff.',
    position: [7.4, 2.2, 5.0],
    color: '#ffd46b',
    kind: 'solar',
    priority: 4,
    ...
    webPlayable: true,
    intelligenceStrings: [
      'Primary playable reference map for the T-Central Hub web scene.',
      'Arma3 CTH enters through this solar-system anchor instead of a detached playable node.',
      'Dyson spheres and blackholes resolve as attached intelligence datapoints for continuity routing.',
    ],
    datapointSources: ['csis', 'ss', 'standalone_blackhole', 'deep_blackhole'],
    playableServerSlugs: ['arma3-cth'],
  },
];
...
const MAP_DATA_ANCHOR_KEY = 'solar_system';
const MAP_VIEW_ORIGIN = [0, 0.4, 0];
const MAP_VIEW_SCALE = [0.22, 0.22, 0.22];
...
function toFiniteVector(value, fallback = [0, 0, 0]) {
...
function getRouteHref(node) {
  if (node.route) return node.route;
  if (node.associatedServer) return `/servers/${node.associatedServer}`;
  if (Array.isArray(node.playableServerSlugs) && node.playableServerSlugs[0]) return `/servers/${node.playableServerSlugs[0]}`;
  return null;
}

export const WORLD_LAYOUT = [...PRIMARY_WORLD_NODES, ...DYNAMIC_SERVER_NODES, ...PERIMETER_BLACKHOLES, ...GENERATED_SATELLITE_NODES];

const WORLD_LAYOUT_BY_KEY = new Map(WORLD_LAYOUT.map((node) => [node.key, node]));
const MAP_DATA_ANCHOR_NODE = WORLD_LAYOUT_BY_KEY.get(MAP_DATA_ANCHOR_KEY) || WORLD_LAYOUT.find((node) => node.playableAnchor) || WORLD_LAYOUT[0];
const MAP_DATA_ANCHOR_POSITION = toFiniteVector(MAP_DATA_ANCHOR_NODE?.position);

export const MAP_DATA_ANCHOR = {
  key: MAP_DATA_ANCHOR_NODE?.key || MAP_DATA_ANCHOR_KEY,
  label: MAP_DATA_ANCHOR_NODE?.label || 'Map Anchor',
  position: [...MAP_DATA_ANCHOR_POSITION],
  mapPosition: [...MAP_VIEW_ORIGIN],
  role: MAP_DATA_ANCHOR_NODE?.anchorRole || 'reference-map',
};

export function getSubSectorAssets(nodes = WORLD_LAYOUT) {
  const safeNodes = Array.isArray(nodes) ? nodes : [];
  return safeNodes
    .filter((node) => node && Array.isArray(node.position))
    .map((node) => {
      const position = toFiniteVector(node.position);
      const offset = position.map((value, index) => value - MAP_DATA_ANCHOR_POSITION[index]);
      const mapPosition = offset.map((value, index) => MAP_VIEW_ORIGIN[index] + value * MAP_VIEW_SCALE[index]);
      const href = getRouteHref(node);
      return {
        key: node.key,
        label: node.label,
        kind: node.kind || 'node',
        color: node.color || '#9dd0ff',
        address: node.address || '',
        description: node.description || '',
        position: [...position],
        physicsPosition: [...position],
        mapPosition: roundVector(mapPosition),
        offsetFromAnchor: roundVector(offset),
        anchorKey: MAP_DATA_ANCHOR.key,
        subSectorKey: `${MAP_DATA_ANCHOR.key}:${node.key}`,
        href,
        external: Boolean(node.external),
        playableAnchor: Boolean(node.playableAnchor),
        playableEntry: node.playableAnchor ? true : node.playableEntry ?? !node.associatedServer,
        attachedTo: node.key === MAP_DATA_ANCHOR.key ? null : MAP_DATA_ANCHOR.key,
...
export const WORLD_SUB_SECTOR_ASSETS = getSubSectorAssets(WORLD_LAYOUT);

export const WORLD_MAP_DATA = {
  anchor: MAP_DATA_ANCHOR,
  scale: [...MAP_VIEW_SCALE],
  viewOrigin: [...MAP_VIEW_ORIGIN],
  subSectorAssets: WORLD_SUB_SECTOR_ASSETS,
};

export function getWorldMapAssetByKey(key, assets = WORLD_SUB_SECTOR_ASSETS) {
  return (Array.isArray(assets) ? assets : []).find((asset) => asset.key === key) || null;
}


export function getDefaultDysonAssets() {
  return PRIMARY_WORLD_NODES.filter((node) => node.kind === 'dyson').map((node) => ({
    ...node,
