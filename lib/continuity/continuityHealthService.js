...
function sanitizeError(error) {
  ...
  const message = String(error?.message || 'Unknown error').replace(/\s+/g, ' ').trim().slice(0, 300);
  return { name, message };
}
...
class ContinuityHealthService {
  ...
  async persistSnapshot(snapshot = this.evaluate()) {
    ...
    try {
      fileSnapshots = await readSnapshots();
      storageTargets.push({
        target: 'localSnapshotFile',
        durability: 'durable',
    ...
    const fileTarget = storageTargets[0];
    try {
      fileSnapshots.unshift(snapshot);
      await writeSnapshots(fileSnapshots);
      fileTarget.write = {
        status: 'success',
        lastSuccessUnix: Math.floor(Date.now() / 1000),
      };
    ...
    storageTargets.push({
      ...
      read: {
        status: this.latestSnapshot ? 'success' : 'degraded',
        lastSuccessUnix: this.latestSnapshot ? this.latestSnapshot.timestampUnix : null,
      },
    });

    const recoverableFromDurableSource = storageTargets.some(
      (target) => target.durability === 'durable' && target.write?.status === 'success'
    );

    const durabilityStatus = recoverableFromDurableSource
      ? 'durable'
      : storageTargets.some((target) => target.write?.status === 'success' || target.read?.status === 'success')
        ? 'ephemeral'
        : 'degraded';
    ...
    this.latestSnapshot = persistedSnapshot;

    return {
      snapshot: persistedSnapshot,
      persistence: {
        recoverableFromDurableSource,
        storageTargets,
      },
    };
  }
