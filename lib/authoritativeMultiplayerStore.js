import { buildGravitySourcesFromSeed, stepFrame } from '@/lib/simCore/stepFrame';
...
function sanitizePlayerForClient(player) {
  return {
    ...
    quantumSignature: player.quantumSignature || 'Q12D-0-0',
...
export function updateAuthoritativePlayer({ roomName = ROOM_NAME, id, token, snapshot = {} } = {}) {
  ...
  player.velocity = sanitizeVector(stepped.velocity, player.velocity).map((value) => Number(value.toFixed(3)));
  ...
  player.quantumSignature = snapshot.quantumSignature || player.quantumSignature;
