...
export function buildOperationsState({
    ...
    withStatus(
      {
        ...
        description: 'Keep mission routing active between deep-space anchors, Dyson spheres, mining seams, and in-sim entropy stabilization.',
      },
      routeTrips >= 1,
  ...
  const objectives = [
    ...
    withStatus(
      {
        id: 'objective-entropy-stabilization',
        title: 'Stabilize entropic cargo',
        description: 'Convert the carried haul into scalar gains through the in-sim entropy resolver.',
      },
      entropyResolved > 0,
      entropyResolved > 0 ? `${entropyResolved} unit${entropyResolved === 1 ? '' : 's'} stabilized for ${credits.toFixed(2)} ${currency.shortLabel}.` : 'Resolve carried entropy inside the sim.'
    ),
  ];
    ...
    withStatus(
      {
        ...
        description: 'Complete the core game fantasy: singleplayer prep → multiplayer mining → entropy stabilization.',
        modes: ['all'],
      },
      soloLaunchReady && multiplayerTransitionReady && entropyMined > 0 && entropyResolved > 0 && exchangeReturnReady,
      entropyResolved > 0
        ? `${credits.toFixed(2)} ${currency.shortLabel} stabilized from resolved entropy.`
        : unresolvedEntropy > 0
    ...
    withStatus(
      {
        ...
        description: 'Complete the website-to-game fantasy: route selection, traversal, mining, stabilization, and return to command shell.',
        modes: ['all'],
      },
  ...
  const missionLoop = [
    ...
    {
      id: 'loop-settle',
      label: 'Entropy stabilization',
      detail: entropyResolved > 0 ? `${credits.toFixed(2)} ${currency.shortLabel} stabilized.` : 'Resolve carried entropy inside the sim.',
  ...
  return {
    ...
    modeSummary:
      lobbyMode === 'hub'
        ? 'Shared route visibility, synchronized pilot presence, discrepant hub-star sync, entropic mining, and in-sim entropy stabilization stay available.'
