...
export function buildOperationsState({
    ...
    withStatus(
      {
        ...
        description: 'Keep mission routing active between deep-space anchors, Dyson spheres, mining seams, and return storage.',
      },
      routeTrips >= 1,
  ...
  const objectives = [
    ...
    withStatus(
      {
        id: 'objective-cargo-storage-return',
        title: 'Store entropy cargo',
        description: 'Bring the haul back through the blackhole fabric and store it in the command vault.',
      },
      entropyResolved > 0,
      entropyResolved > 0 ? `${entropyResolved} unit${entropyResolved === 1 ? '' : 's'} stored for ${credits.toFixed(2)} ${currency.shortLabel}.` : 'Return cargo to the command vault and resolve entropy.'
    ),
  ];
    ...
    withStatus(
      {
        ...
        description: 'Complete the core game fantasy: singleplayer prep → multiplayer mining → command-vault storage.',
        modes: ['all'],
      },
      soloLaunchReady && multiplayerTransitionReady && entropyMined > 0 && entropyResolved > 0,
      entropyResolved > 0
        ? `${credits.toFixed(2)} ${currency.shortLabel} logged from stored entropy.`
        : unresolvedEntropy > 0
    ...
    withStatus(
      {
        ...
        description: 'Complete the website-to-game fantasy: route selection, traversal, mining, storage, and return to command shell.',
        modes: ['all'],
      },
  ...
  const missionLoop = [
    ...
    {
      id: 'loop-store',
      label: 'Cargo storage return',
      detail: entropyResolved > 0 ? `${credits.toFixed(2)} ${currency.shortLabel} logged in the command vault.` : 'Return cargo to the command vault to resolve entropy.',
  ...
  return {
    ...
    modeSummary:
      lobbyMode === 'hub'
        ? 'Shared route visibility, synchronized pilot presence, discrepant hub-star sync, entropic mining, and command-vault storage stay available.'
