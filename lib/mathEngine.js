...
export function projectNDToColor(dimensions = [], config = {}) {
  const source = Array.isArray(dimensions) ? dimensions : [];
  const length = Math.max(source.length, 1);
  const pickIndex = (offset = 0) => Math.min(length - 1, Math.floor((length - 1) * offset));

  const redIndex = Number.isInteger(config.redIndex) ? config.redIndex : pickIndex(0);
  const greenIndex = Number.isInteger(config.greenIndex) ? config.greenIndex : pickIndex(0.4);
  const blueIndex = Number.isInteger(config.blueIndex) ? config.blueIndex : pickIndex(0.8);

  const red = source[redIndex] || 0;
  const green = source[greenIndex] || 0;
  const blue = source[blueIndex] || 0;

  const redBase = Number.isFinite(config.redBase) ? config.redBase : 0.45;
  const greenBase = Number.isFinite(config.greenBase) ? config.greenBase : 0.45;
  const blueBase = Number.isFinite(config.blueBase) ? config.blueBase : 0.55;

  const redGain = Number.isFinite(config.redGain) ? config.redGain : 0.4;
  const greenGain = Number.isFinite(config.greenGain) ? config.greenGain : 0.4;
  const blueGain = Number.isFinite(config.blueGain) ? config.blueGain : 0.35;

  return new THREE.Color(
    clamp(redBase + red * redGain, 0.1, 1),
    clamp(greenBase + green * greenGain, 0.1, 1),
    clamp(blueBase + blue * blueGain, 0.1, 1),
  );
}
