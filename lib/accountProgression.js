...
export function normalizeSnapshotSavedAt(savedAt = null) {
  const timestamp = Date.parse(savedAt || '');
  return Number.isFinite(timestamp) ? new Date(timestamp).toISOString() : new Date().toISOString();
}

export function getSnapshotSavedAtMs(snapshot = {}) {
  const timestamp = Date.parse(snapshot?.savedAt || snapshot?.saved_at || '');
  return Number.isFinite(timestamp) ? timestamp : null;
}

export function isOlderProgressSnapshot(incomingSnapshot = {}, currentSnapshot = {}) {
  const incomingSavedAt = getSnapshotSavedAtMs(incomingSnapshot);
  const currentSavedAt = getSnapshotSavedAtMs(currentSnapshot);
  return incomingSavedAt !== null && currentSavedAt !== null && incomingSavedAt < currentSavedAt;
}
...
export function buildAccountSnapshot({ identity = null, steamUser = null, progress = defaultProgressState(), savedAt = null } = {}) {
  ...
  const normalizedSavedAt = normalizeSnapshotSavedAt(savedAt);
  return {
    version: ACCOUNT_PROGRESS_VERSION,
    identity: {
