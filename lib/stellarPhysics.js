...
export function buildDysonEncryptionState({ epochAnchor, stellarProfile, seed = 'synaptics-dyson' } = {}) {
  const hash = hash53(`${seed}:${epochAnchor?.unix || 0}:${stellarProfile?.spectralClass || 'G'}`);
  const matrix = epochAnchor?.matrixScalar || 0.5;
  const sidereal = epochAnchor?.siderealDrift || 0.5;
  const dysonAlignment = epochAnchor?.dysonAlignment || 0.5;
  const encryptionStrength = clamp(0.62 + matrix * 0.24 + sidereal * 0.14, 0, 1);
  const ringPhase = ((hash % 6283) / 1000);

  return {
    ringOneLabel: 'Collector ring',
    ringTwoLabel: 'Habitat ring',
    ringThreeLabel: 'Encryption ring',
    encryptionStrength: round(encryptionStrength, 4),
    keyspacePetabits: round(512 + encryptionStrength * 1536, 1),
    stellarFeedLumens: round((stellarProfile?.luminositySolar || 1) * 1000 * (0.82 + dysonAlignment * 0.24), 1),
    latticePhase: round(ringPhase, 4),
  };
}
